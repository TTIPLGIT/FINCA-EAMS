...
class Asset extends Depreciable
{
    ...
    */
    ...
    protected $rules = [
        ...
        'costcentre_id' => 'numeric|nullable',
   ...
   **/
    ...
    {
        return $this->belongsTo('\App\Models\CostCentre', 'costcentre_id');
    }
  ...
  */
    ...
    public function scopeByFilter($query, $filter)
    {
        return $query->where(function ($query) use ($filter) {
            foreach ($filter as $key => $search_val) {
                ...
                if ($fieldname =='costcentre') {
                    $query->where(function ($query) use ($search_val) {
                        $query->whereHas('costcentre', function ($query) use ($search_val) {
                            $query->where('costcentres.costcode', 'LIKE', '%' . $search_val . '%');
                        });
                    });
                }
            }
    ...
    */
    ...
    /**
     * Query builder scope to order on costcentre name
     *
     * @param  \Illuminate\Database\Query\Builder  $query  Query builder instance
     * @param  text                              $order       Order
     *
     * @return \Illuminate\Database\Query\Builder          Modified query builder
     */
    public function scopeOrderCostCentre($query, $order)
    {
        return $query->leftJoin('costcentres as costcentres_assets', 'assets.costcentre_id', '=', 'costcentres_assets.id')->orderBy('costcentres_assets.costcode', $order);
    }
