...
class Helper
{
    ...
    public static function checkLowInventory()
    {
        ...
        $electronics = Electronics::withCount('users')->whereNotNull('min_amt')->get();
        ...
        foreach ($electronics as $electronic) {
            $avail = $electronic->qty - $electronic->users_count;
            if ($avail < ($electronic->min_amt) + \App\Models\Setting::getSettings()->alert_threshold) {

                if ($electronic->qty > 0) {
                    $percent = number_format((($avail / $electronic->qty) * 100), 0);
                } else {
                    $percent = 100;
                }

                $items_array[$all_count]['id'] = $electronic->id;
                $items_array[$all_count]['name'] = $electronic->name;
                $items_array[$all_count]['type'] = 'electronics';
                $items_array[$all_count]['percent'] = $percent;
                $items_array[$all_count]['remaining'] = $avail;
                $items_array[$all_count]['min_amt']=$electronic->min_amt;
                $all_count++;
            }
        ...
        foreach ($furnitures as $furniture) {
            $avail = $furniture->qty - $furniture->users_count;
            if ($avail < ($furniture->min_amt) + \App\Models\Setting::getSettings()->alert_threshold) {

                if ($furniture->qty > 0) {
                    $percent = number_format((($avail / $furniture->qty) * 100), 0);
                } else {
                    $percent = 100;
                }

                $items_array[$all_count]['id'] = $furniture->id;
                $items_array[$all_count]['name'] = $furniture->name;
                $items_array[$all_count]['type'] = 'furnitures';
                $items_array[$all_count]['percent'] = $percent;
                $items_array[$all_count]['remaining'] = $avail;
                $items_array[$all_count]['min_amt']=$furniture->min_amt;
                $all_count++;
            }
        ...
        foreach ($buildings as $building) {
            $avail = $building->qty - $building->users_count;
            if ($avail < ($building->min_amt) + \App\Models\Setting::getSettings()->alert_threshold) {

                if ($building->qty > 0) {
                    $percent = number_format((($avail / $building->qty) * 100), 0);
                } else {
                    $percent = 100;
                }

                $items_array[$all_count]['id'] = $building->id;
                $items_array[$all_count]['name'] = $building->name;
                $items_array[$all_count]['type'] = 'buildings';
                $items_array[$all_count]['percent'] = $percent;
                $items_array[$all_count]['remaining'] = $avail;
                $items_array[$all_count]['min_amt']=$building->min_amt;
                $all_count++;
            }
