...
/*                Mail::send('emails.send-login', $data, function ($m) use ($user) {
    ...
    * @return \Illuminate\Http\RedirectResponse
    ...
    public function destroy($id = null)
    {
        try {
            ...
            if ($user->electronics()->count() > 0) {
                // Redirect to the user management page
                return redirect()->route('users.index')->with('error', 'This user still has ' . $user->electronics()->count() . ' electronics associated with them.');
            }

             if ($user->furnitures()->count() > 0) {
                // Redirect to the user management page
                return redirect()->route('users.index')->with('error', 'This user still has ' . $user->furnitures()->count() . ' furnitures associated with them.');
            }

             if ($user->infrastructures()->count() > 0) {
                // Redirect to the user management page
                return redirect()->route('users.index')->with('error', 'This user still has ' . $user->infrastructures()->count() . ' infrastructures associated with them.');
            }
    ...
    * @return \Illuminate\Contracts\View\View
    ...
    public function postBulkEdit(Request $request)
    {
        ...
        if (($request->has('ids')) && (count($request->input('ids')) > 0)) {
            ...
            $users = User::whereIn('id', $user_raw_array)->with('groups', 'assets', 'licenses', 'accessories','electronics','furnitures','infrastructures')->get();
        ...
        if ((!$request->has('ids')) || (count($request->input('ids')) == 0)) {
            ...
            if (!config('app.lock_passwords')) {
                ...
                foreach ($electronics as $electronic) {
                    $electronic_array[] = $electronic->electronics_id;
                    // Update the asset log
                    $logAction = new Actionlog();
                    $logAction->item_id = $electronic->id;
                    $logAction->item_type = Electronics::class;
                    $logAction->target_id = $electronic->assigned_to;
                    $logAction->target_type = User::class;
                    $logAction->user_id = Auth::user()->id;
                    $logAction->note = 'Bulk checkin electronic and delete user';
                    $logAction->logaction('checkin from');
                }


                foreach ($infrastructures as $infrastructure) {
                    $infrastructure_array[] = $infrastructure->infrastructures_id;
                    // Update the asset log
                    $logAction = new Actionlog();
                    $logAction->item_id = $infrastructure->id;
                    $logAction->item_type = Infrastructures::class;
                    $logAction->target_id = $infrastructure->assigned_to;
                    $logAction->target_type = User::class;
                    $logAction->user_id = Auth::user()->id;
                    $logAction->note = 'Bulk checkin infrastructure and delete user';
                    $logAction->logaction('checkin from');
                }
    ...
    * @return \Illuminate\Contracts\View\View
    ...
    {
        if(!$user = User::with('assets', 'assets.model', 'consumables', 'accessories', 'electronics', 'infrastructures', 'licenses', 'userloc')->withTrashed()->find($userId)) {
            $error = trans('admin/users/message.user_not_found', compact('id'));
            // Redirect to the user management page
            return redirect()->route('users.index')->with('error', $error);
        }
    ...
    * @return \Illuminate\Http\RedirectResponse
    ...
    public function getExportUserCsv()
    {
        ...
        $response = new StreamedResponse(function () {
            ...
            User::with('assets', 'accessories', 'electronics', 'infrastructures', 'consumables', 'department', 'licenses', 'manager', 'groups', 'userloc', 'company','throttle')->orderBy('created_at', 'DESC')->chunk(500, function($users) use($handle) {
                $headers=[
                ...
                foreach ($users as $user) {
                    ...
                    $values = [
                        ...
                        $user->electronics->count(),
                        $user->infrastructures->count(),
    ...
    public function printInventory($id)
    {
        ...
        $electronics = $show_user->electronics()->get();
        $infrastructures = $show_user->infrastructures()->get();
